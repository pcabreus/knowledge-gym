# Questions Index

## Practice flow (start here)
1) Pick a topic from topics/_index.md.
2) Do 1 MCQ (4–5 min) + 1 open/design (8–12 min).
3) Answer in BLUF: definition → decision rules → trade-offs → example → failure mode.
4) I will score using Prompt C and give a targeted trap question.

## Event-driven architecture & reliability
- [Message delivery semantics — MCQ 1](message-delivery-semantics-mcq-1.md)
- [Idempotency — Open 1](idempotency-open-1.md)
- [Retries & backoff — MCQ 1](retries-backoff-mcq-1.md)
- [Timeouts — MCQ 1](timeouts-mcq-1.md)
- [Circuit breaker — MCQ 1](circuit-breaker-mcq-1.md)
- [Backpressure — MCQ 1](backpressure-mcq-1.md)
- [DLQ & poison messages — MCQ 1](dlq-poison-mcq-1.md)
- [Third-party integration resilience — Open 1](third-party-integration-open-1.md)

## Microservices & communication
- [Bounded contexts — Open 1](bounded-contexts-open-1.md)
- [Microservices design — Open 1](microservices-design-open-1.md)
- [Sync vs async — Open 1](sync-vs-async-open-1.md)
- [Versioning APIs & events — MCQ 1](versioning-apis-events-mcq-1.md)
